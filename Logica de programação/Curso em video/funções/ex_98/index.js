/*  Crie um programa que tenha uma função SuperSomador(), que vai receber dois números como parâmetro e depois vai retornar a soma de todos os valores no intervalo entre os valores recebidos.
Ex: SuperSomador(1, 6) vai somar 1 + 2 + 3 + 4 + 5 + 6 e vai retornar 21
SuperSomador(15, 19) vai somar 15 + 16 + 17 + 18 + 19 e vai retornar 85 */

let result = document.querySelector('#resultado');

function superSomador() {
    let inicio = parseInt(prompt('Vamos somar um intervalo de números.\nDigite o número INICIAL: '));
    let final = parseInt(prompt('Digite o número FINAL desse intervalo: '));
    let mostrar = "";
    let soma = 0;

    for (let i = inicio; i <= final; i++) {
        mostrar += i;

        if (i < final) {
            mostrar += " + ";
        }

        soma += i;
    }

    result.textContent = `A soma de ${mostrar} = ${soma}`;
}

superSomador();


